...
class TestEmissaoBilheteria:
    ...
    async def test_emitir_ingresso_success(self, fake_db, mock_get_database, sample_evento, 
        ...
        # não foi enviada ilha, portanto não deve aparecer no resultado
        assert result.get("ilha_id") is None

    @pytest.mark.asyncio
    async def test_emitir_ingresso_success_with_ilha(self, fake_db, mock_get_database, sample_evento,
                                                     sample_tipo_ingresso, sample_participante, sample_ilha,
                                                     mock_verify_bilheteria):
        """Emissão com ilha especificada devolve campo no response e grava corretamente."""
        from app.routers.bilheteria import EmissaoRequest
        ...
        assert result.get("ilha_id") == str(sample_ilha["_id"])
        # ingressos_emitidos collection deve conter registro com ilha_id
        assert fake_db.ingressos_emitidos.docs[0].get("ilha_id") == str(sample_ilha["_id"])
    ...
    @pytest.mark.asyncio
    async def test_emitir_ingresso_ilha_cheia(self, fake_db, mock_get_database, sample_evento,
                                               sample_tipo_ingresso, sample_participante,
                                               sample_ilha, mock_verify_bilheteria):
        """Não deve permitir emissão quando a capacidade da ilha estiver atingida."""
        from app.routers.bilheteria import EmissaoRequest

        # ajustar capacidade pequena
        ilha = dict(sample_ilha)
        ilha["capacidade_maxima"] = 1
        fake_db.eventos.docs.append(sample_evento)
        fake_db.ilhas.docs.append(ilha)
        fake_db.tipos_ingresso.docs.append(sample_tipo_ingresso)
        fake_db.participantes.docs.append(sample_participante)
        # já existe um ingresso para a ilha
    ...
    @pytest.mark.asyncio
    async def test_ilha_stats(self, fake_db, mock_get_database, sample_evento, sample_ilha, mock_verify_bilheteria):
        """Rota de estatísticas deve retornar capacidade e número de ingressos."""
        fake_db.eventos.docs.append(sample_evento)
        fake_db.ilhas.docs.append(sample_ilha)
        # um ingresso emitido nesta ilha
        fake_db.ingressos_emitidos.docs.append({
            "_id": ObjectId(),
            "ilha_id": str(sample_ilha["_id"]),
            "evento_id": str(sample_evento["_id"]),
        ...
        stats = await bilheteria.ilha_stats(
            str(sample_ilha["_id"]),
            evento_id=str(sample_evento["_id"])
        )
        assert stats["ilha_id"] == str(sample_ilha["_id"])
